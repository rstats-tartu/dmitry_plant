#change working directory
setwd("C:\Users\dmitry\Desktop\Documents\project_HT1 mutant phenotyping\____Experiments\exp 1 Aug 2018\Data from from Mirko")

#Load libraries
require(readxl)
require(dplyr)
require(tidyr)
require(ggplot2)
require(lubridate)

#import data
fc = read_xlsx("Fc_Plant.xlsx") %>% separate(plant_name, c("co2", "temp", "water", "light"), sep = "_")
ir = read_xlsx("Ir_Plant.xlsx") %>% separate(plant_name, c("co2", "temp", "water", "light"), sep = "_")
drgb = read_xlsx("Rgb_Morpho_Plant.xlsx") %>% separate(plant_name, c("co2", "temp", "water", "light"), sep = "_")
sc = read_xlsx("ScalesMeasure.xlsx") %>% separate(plant_name, c("co2", "temp", "water", "light"), sep = "_")
irb = read_xlsx("Ir_Plant_before.xlsx") %>% separate(plant_name, c("co2", "temp", "water", "light"), sep = "_")


fc$date = as.character(fc$date)
ir$date = as.character(ir$date)
irb$date = as.character(irb$date)
drgb$date = as.character(drgb$date)
sc$date = as.character(sc$date)

fc = separate(fc, date, c("day", "hours"), sep = " ") %>% select(-hours, -time)
fc$day = ymd(fc$day)

ir = separate(ir, date, c("day", "hours"), sep = " ") %>% select(-hours, -time)
ir$day = ymd(ir$day)

irb = separate(irb, date, c("day", "hours"), sep = " ") %>% select(-hours, -time)
irb$day = ymd(irb$day)

drgb = separate(drgb, date, c("day", "hours"), sep = " ") %>% select(-hours, -time)
drgb$day = ymd(drgb$day)

sc$date = ymd(sc$date)
sc$day = sc$date
sc = select(sc, -date, - time)

#plots

fc %>%
  filter(day > "2018-08-04")%>%
  ggplot(aes(day, QY_max, color = genotype))+
  stat_summary(fun.y = mean, geom = "line")+
  stat_summary(fun.data = mean_se, geom = "pointrange")+
  facet_grid(water ~ light)+
  theme_bw()

fc %>%
  filter(day > "2018-08-04")%>%
  ggplot(aes(day, QY_Lss, color = genotype))+
  stat_summary(fun.y = mean, geom = "line")+
  stat_summary(fun.data = mean_se, geom = "pointrange")+
  facet_grid(water ~ light)+
  theme_bw()

fc %>%
  filter(day > "2018-08-04")%>%
  ggplot(aes(day, NPQ_Lss, color = genotype))+
  stat_summary(fun.y = mean, geom = "line")+
  stat_summary(fun.data = mean_se, geom = "pointrange")+
  facet_grid(water ~ light)+
  theme_bw()

fc %>%
  filter(day > "2018-08-04")%>%
  ggplot(aes(day, Rfd_Lss, color = genotype))+
  stat_summary(fun.y = mean, geom = "line")+
  stat_summary(fun.data = mean_se, geom = "pointrange")+
  facet_grid(water ~ light)+
  theme_bw()

ir %>%
  filter(day != "2018-08-06")%>%
  ggplot(aes(day, `Temp-avg`, color = genotype))+
  stat_summary(fun.y = mean, geom = "line")+
  stat_summary(fun.data = mean_se, geom = "pointrange")+
  facet_grid(water ~ light)+
  theme_bw()

irb %>%
  filter(day != "2018-08-06")%>%
  ggplot(aes(day, `Temp-avg`, color = genotype))+
  stat_summary(fun.y = mean, geom = "line")+
  stat_summary(fun.data = mean_se, geom = "pointrange")+
  facet_grid(water ~ light)+
  theme_bw()

drgb %>%
  ggplot(aes(day, AREA_MM, color = genotype))+
  stat_summary(fun.y = mean, geom = "line")+
  stat_summary(fun.data = mean_se, geom = "pointrange")+
  facet_grid(water ~ light)+
  theme_bw()

sc %>%
  filter(genotype != "empty")%>%
  ggplot(aes(day, weight, color = plant_id))+
  geom_line()+
  facet_grid(water ~ light)+
  theme_bw()+
  guides(color = FALSE)


