...
```{r}
fc <- read_delim("data/fc_plant.csv", delim = ";", locale = locale(decimal_mark = ","))
...
ggplot(data = fc) +
  ...
  facet_wrap(~ str_c(tray_id, cond, sep = " ")) +
  labs(caption = "QY_max - max quantum yield of PSII per tray.")
ggsave("output/qymax_per_tray.png")
```
...
ggplot(data = fc) +
  ...
  labs(caption = "QY_max - max quantum yield of PSII per environmental condition. Color, plant genotype.")
ggsave("output/qymax_per_env.png")
```
...
ggplot(data = fc) +
  ...
  scale_color_viridis_d() +
  labs(caption = "QY_max - max quantum yield of PSII per tray position. Color, plant genotype.")
ggsave("output/qymax_per_pos.png")
```
...
```{r}
ir <- read_delim("data/ir_plant.csv", delim = ";", locale = locale(decimal_mark = ","))
...
## Morpho - area in mm
```{r}
morpho <- read_delim("data/rgb_morpho.csv", delim = ";", locale = locale(decimal_mark = ","))
...
```{r}
measure <- read_delim("data/scales_measure.csv", delim = ";", locale = locale(decimal_mark = ","))
